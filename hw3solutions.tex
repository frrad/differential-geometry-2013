...
\section{A Riemannian manifold is said to be homogeneous if the isometry group acts transitively. Show that homogeneous manifolds are geodescially complete.}

\section{Let $N \subset (M,g)$ be a submanifold. Let $\nabla^N$ denote the connection on $N$ that comes from teh metric induced by $g$. Define the second fundamental form of $N$ in $M$ by $$\2(X,Y) = \nabla_X^N Y - \nabla_X Y$$ Show that $\2 =0$ on $N$ iff $N$ is totally geodesic. (The definition of \emph{totally geodesic} is on p. 145)}
